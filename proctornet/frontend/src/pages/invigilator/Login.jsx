...
import { ArrowRight, Lock, Hash, Key, AlertCircle, Eye, EyeOff, WifiOff, X } from 'lucide-react'
...
export default function InvigilatorLogin() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.examId || !form.invId || !form.invPassword) {
      setErrorState({
        type: 'validation',
        title: 'Missing Required Fields',
        message: 'Please provide the Exam ID, Invigilator ID, and Session Password.'
      })
      return
    }
  ...
  return (
    <AuthLayout
      role="invigilator"
      title="Invigilator Session Access"
      subtitle="Enter your assigned exam session details to start live lab supervision."
      heroHeadline="Invigilator Login"
      heroSubtitle="Real-time multi-seat video matrix, anomaly detection, and candidate supervision."
      personImage="/login-invigilator.jpg"
      ...
      <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <Hash size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9ca3af]" />
            <input
              type="text"
              name="examId"
              value={form.examId}
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <Key size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9ca3af]" />
            <input
              type="text"
              name="invId"
              value={form.invId}
              onChange={setField('invId')}
        ...
        >
          <span>{loading ? 'Authenticating Session…' : 'Access Live Grid'}</span>
          <ArrowRight size={16} />
        </button>
      </form>
    </AuthLayout>
