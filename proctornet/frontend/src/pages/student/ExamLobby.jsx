...
import {
  ...
  ArrowRight, RefreshCw, UserCheck, BookOpen
} from 'lucide-react'
  ...
  if (loading) {
    return (
      <DashboardLayout title="Exam Lobby">
        <div className="p-12 text-center text-xs font-mono text-slate-500">Loading exam lobby…</div>
      </DashboardLayout>
    )
  }
  ...
  return (
    <DashboardLayout title="Exam Lobby">
      <div className="max-w-4xl mx-auto py-4 space-y-5 font-sans">
        ...
        <Card className="bg-[#141416] border-[#27272A] p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#27272A] pb-4 mb-4">
            <div>
              ...
              <p className="text-xs font-mono text-slate-400 mt-0.5">Subject Code: {exam?.subject || 'CS301'}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Timer Block */}
            <div className="p-4 bg-[#09090B] border border-[#27272A] rounded-2xl text-center flex flex-col items-center justify-center">
              <span className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <Clock size={12} /> {isOver ? 'Exam Status' : timeToStart === 0 ? 'Exam is Live' : 'Countdown to Exam Start'}
              </span>
              {isOver ? (
        ...
        <Card className="bg-[#141416] border-[#27272A] p-6 space-y-4">
          <h2 className="text-sm font-bold text-slate-100 flex items-center gap-2">
            <Shield className="w-4 h-4 text-indigo-400" /> Mandatory BYOD Security Protocol
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono">
            <div className="p-3 rounded-xl bg-[#09090B] border border-[#27272A] flex items-center gap-2">
              <Camera size={14} className="text-indigo-400" />
              <span className="text-slate-300">Continuous AI Face Proctoring</span>
            </div>
            <div className="p-3 rounded-xl bg-[#09090B] border border-[#27272A] flex items-center gap-2">
              <Shield size={14} className="text-indigo-400" />
              <span className="text-slate-300">Browser Kiosk Fullscreen Lock</span>
            </div>
            <div className="p-3 rounded-xl bg-[#09090B] border border-[#27272A] flex items-center gap-2">
              <Wifi size={14} className="text-indigo-400" />
              <span className="text-slate-300">Secure Network Verification</span>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-[#27272A] flex flex-col sm:flex-row items-center justify-between gap-3">
            <Button
              ...
              className="w-full sm:w-auto text-xs font-mono border-[#27272A] bg-[#09090B]"
            >
              Run BYOD Device Diagnostic
            </Button>
            <Button
              onClick={() => navigate(`/student/exams/${examId}/security`)}
              className="w-full sm:w-auto text-xs font-mono font-bold bg-indigo-600 hover:bg-indigo-500 text-white px-6"
            >
              Proceed to Exam Check-in <ArrowRight size={14} className="ml-1.5" />
            </Button>
          </div>
        </Card>
      </div>
